...
### document-refiner + code-reviewer 2段階検証パターン

品質検証フェーズでは、以下の順序でエージェントを実行する：

1. **document-refiner**: ドキュメントを直接修正（write権限あり）
2. **code-reviewer**: 修正内容をレビュー（パス切り詰め、構文エラー等を検出）
3. **必要に応じて追加修正**: レビュー指摘を適用
4. **gap-detector**: 再検証して削減率を測定

**重要**: document-refiner単独では検出できない問題（パス切り詰めバグ `02_operatio..` など）がcode-reviewerで発見されることがある。必ず2段階で検証する。

### 並列実行パターン

- 異なるファイルを対象とするタスクは並列実行可能
- 例: 3つのdocument-refinerを同時起動（各々異なるファイルを修正）
- 同様に3つのcode-reviewerも同時起動可能

### 意図的に残すべきギャップマーカー

すべてのマーカーを解消すべきではない。以下は意図的に残す：

| マーカー | 残す理由 |
|----------|----------|
| `LINK_NEEDED` | 未作成ドキュメントへのリンク（将来フェーズで作成予定） |
| `SME_NEEDED` | 専門家（CFD/GIS等）の判断が必要な選択 |
| `ASSUMPTION` | 明示的に仮定を示すマーカー（情報提供目的） |

**ベストプラクティス**: 大量のTODOCSマーカーは「Phase N: 具体的タスク」形式に変換することで、マーカー数を削減しつつトレーサビリティを維持できる。
