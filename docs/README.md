...
3docaは**「エージェント支援による理解しやすいドキュメント作成」**のためのフレームワークです。
AIエージェント（Claude Code等）と人間が協働してドキュメントを作成・管理するための構造とルールを提供します。

- **Diátaxis**: 学習・タスク・リファレンス・説明の4象限で情報を整理
- **運用ドキュメント**: プロセス・プレイブック・ランブックで実行を支援
- **C4モデル**: アーキテクチャを段階的な詳細度で可視化

### 自己言及的な実演

このドキュメント群自体が、フレームワークの**実例かつ実演**として機能します。

- テンプレートを使ってドキュメントを作成する方法を説明するドキュメントが、そのテンプレートを使って作成されている
- ギャップマーカーの使い方を説明するドキュメントに、実際のギャップマーカーが含まれている
- 品質検証プロセスを説明するドキュメントが、そのプロセスで検証されている

### 適用可能性

このフレームワークは技術ドキュメントに限らず、**汎用的に使用**できます。

- **ソフトウェアプロジェクト**: API仕様、運用手順、アーキテクチャ設計
- **研究プロジェクト**: 実験手順、データ処理、結果の解釈
- **業務プロセス**: 標準作業手順、トラブルシューティング、引き継ぎ資料

フレームワークを学び、あなた自身のプロジェクトに適用してください。
