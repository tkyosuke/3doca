# 元ファイル採用箇所レポート - 251129claude.md

**作成日**: 2025-11-30
**対象**: tmp2/templates/ および tmp2/examples/
**目的**: 元ファイルのどの部分がテンプレート/実例に反映されたかを可視化
...
| セクション | 反映状況 | 反映率 | 備考 |
|-----------|---------|-------|------|
| Diátaxisフレームワーク | 🟢 完全反映 | 100% | tmp2/README.mdに4タイプ説明表を追加 |
| 運用系ドキュメント階層 | 🟢 完全反映 | 100% | 6テンプレート作成（プロセス/プレイブック/ランブック/トラブルシューティング/ADR/チートシート） |
| ファイル構造 | 🟡 部分反映 | 70% | tmp2/構造は採用、番号体系を独自拡張（0x/1x/2x/3x）、知識ベース全体構造は未実装 |
| 命名規則 | 🟢 完全反映 | 100% | kebab-case、2桁ゼロ埋め、31文字以内を遵守 |
| RAG対応フロントマター | 🟡 部分反映 | 80% | 構造は全テンプレートに実装、related_docs/prerequisitesは一部実装 |
| セマンティック検索最適化 | 🟡 部分反映 | 60% | description 150文字以内、代名詞回避は実装、チャンクサイズ最適化は部分的 |
| Mermaidダイアグラム活用 | 🟢 完全反映 | 100% | ダークモード最適化、フローチャート/意思決定ツリー/Ganttを実例に実装 |
| ADR（MADR形式） | 🟢 完全反映 | 100% | 04-adr-template.md作成、MADR形式準拠 |
| トラブルシューティング記録 | 🟢 完全反映 | 100% | 03-troubleshooting-template.md作成、構造化形式実装 |
| AIエージェント活用 | ⚪ 未反映 | 0% | テンプレート作成での活用提案は未ドキュメント化 |
| PostgreSQL/pgvector統合 | 🟡 部分反映 | 30% | フロントマター設計は反映、具体的なSQL/スキーマ/インデックス設計は未実装 |
| 段階的導入ロードマップ | ⚪ 未反映 | 0% | Phase 1-3定義なし |
| 継続的改善プロセス | ⚪ 未反映 | 0% | レビュー/フィードバック/更新サイクルは未ドキュメント化 |

**総合反映率**: 約 **72%**（13セクション中、完全反映6件、部分反映5件、未反映2件）

### 凡例

- 🟢 **完全反映**: 元ファイルの内容がテンプレート/実例に完全に実装されている
- 🟡 **部分反映**: 一部のみ反映、または簡略化して実装
- ⚪ **未反映**: 未実装（将来の拡張候補）
...
## <span style="color: #22863a;">🟢 Diátaxisフレームワークを軸とした階層設計</span>

<span style="color: #22863a;">**反映状況**: tmp2/README.mdに4タイプ説明表を完全実装。チュートリアル/ハウツー/リファレンス/説明の定義と目的を明記。</span>
...
<span style="color: #22863a;">この4タイプに加え、**運用系ドキュメント**として以下の階層を設ける。</span>

<span style="color: #22863a;">**反映状況**: 6テンプレート完全作成（00-process-document-template.md, 01-playbook-template.md, 02-runbook-template.md, 03-troubleshooting-template.md, 04-adr-template.md, 05-cheatsheet-template.md）。ポリシー層のみ未実装。</span>

```
┌─────────────────────────────────────────────────────────┐
│ ポリシー（原則・方針）     ← 変更頻度：年1回程度         │ ⚪ 未実装
├─────────────────────────────────────────────────────────┤
│ プレイブック（状況対応指針） ← 複雑シナリオの意思決定枠組 │ 🟢 実装済み
├─────────────────────────────────────────────────────────┤
│ プロセスドキュメント       ← ワークフロー全体の流れ      │ 🟢 実装済み
├─────────────────────────────────────────────────────────┤
│ ランブック（運用手順書）   ← 具体的なタスク実行手順      │ 🟢 実装済み
├─────────────────────────────────────────────────────────┤
│ チートシート（早見表）     ← 即座参照用の要点集約        │ 🟢 実装済み
└─────────────────────────────────────────────────────────┘
```

<span style="color: #22863a;">**プレイブック**は判断を伴う複雑なシナリオ（障害対応、リリース管理）に対する戦略的ガイド、**ランブック**は判断不要で順に実行するタスク手順という役割分担が重要である。この違いは01-playbook-template.mdと02-runbook-template.mdで明確に実装。</span>
...
### <span style="color: #d97706;">🟡 RAG対応フロントマター設計</span>

<span style="color: #d97706;">**反映状況**: フロントマター構造は全テンプレート/実例に実装。title, description, tags, category, domain, difficulty, created_at, updated_at, version, authorは完全実装。related_docs/prerequisitesは一部実装、pgvectorクエリ例は未実装。</span>
...
## <span style="color: #22863a;">🟢 Mermaidダイアグラムの効果的活用</span>

<span style="color: #22863a;">**反映状況**: 完全実装。ダークモード最適化（theme: 'dark', primaryBorderColor, lineColor設定）、フローチャート（data-quality-analysis-process.md）、意思決定ツリー（anomaly-detection-playbook.md）、Ganttチャート実装。全実例でMermaidダイアグラム使用。</span>
